...
export default {
	...
	computed: {
		...
		highlightContent() {
			...
			codeHighlight = codeHighlight.replace(/^\[(x|@|~|\s)\] (.*)/gm, (match, p1, p2) => {
				...
				return `<span class="task ${type}" data-slug="${encodeURIComponent(
					dataSlug
				)}"><span class="checkbox">[${p1}]</span><span> ${p2}</span></span>`;
			});
			...
			this.$nextTick(() => {
				document
					...
					.forEach((el) => el.addEventListener('click', (_) => this.changeTaskStatus(event)));
			});
	...
	methods: {
		...
		changeTaskStatus(event) {
			...
			const content = decodeURIComponent(event.target.parentNode.getAttribute('data-slug'))
				.replace(/@-/g, '@')
				.replace(/!-/g, '!');
